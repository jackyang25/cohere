...
def Fired (R : List (Rule Fact Action)) (F : FactSet Fact) : Rule Fact Action -> Prop :=
  fun r => r ∈ R ∧ Fires r F

/-- Shadowed rules as a logical set (`Rule -> Prop`). -/
def ShadowedSet (R : List (Rule Fact Action)) (F : FactSet Fact) : Rule Fact Action -> Prop :=
...
-/
def Derive (R : List (Rule Fact Action)) (F : FactSet Fact) : Verdict Action -> Prop :=
  fun v =>
    ∃ r, r ∈ R ∧
      Fires r F ∧
      ¬ Shadowed R F r ∧
